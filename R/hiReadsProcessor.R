...
#'  sector  region/quadrant of the sequencing plate the sample comes from.
#'  barcode unique 4-12bp DNA sequence which identifies the sample
#'  primerltrsequence   DNA sequence of the viral LTR primer with viral LTR sequence following the primer landing site
#'  samplename  Name of the sample associated with the barcode
#'  sampledescription   Detailed description of the sample
#'  gender  sex of the sample: male or female
#'  species species of the sample: homo sapien, mus musculus, etc.
#'  freeze UCSC freeze to which the sample should be aligned to.
#'  linkersequence  DNA sequence of linker adaptor to found following the genomic sequence.
#'  restrictionenzyme   Restriction enzyme used for digestion and sample recovery. Can also be Fragmentase!
...
#' @param gfServerOpts a character vector of options to be passed to gfServer command on top of server defaults. Default: c(repMatch=112312, stepSize=5, tileSize=8)
...
startgfServer <- function(host="localhost", port=5560, seqDir=NULL, gfServerOpts=c(repMatch=112312, stepSize=5, tileSize=8), waitTime=120) {
    if(is.null(seqDir)) {
...
#'
#' @seealso \code{\link{pairwiseAlignSeqs}}, \code{\link{vpairwiseAlignSeqs}}, \code{\link{startgfServer}}, \code{\link{stopgfServer}}, \code{\link{blatSeqs}}, \code{\link{read.psl}}, \code{\link{pslToRangedData}}
#'
...
#' @param port the same number you started the gfServer with. Required if standaloneBlat=FALSE. Default is NULL.
...
#' @param blatParameters a character vector of options to be passed to gfClient/BLAT command except for output file format (psl) and nohead. Default: c(minIdentity=70, minScore=5, stepSize=5, tileSize=8, repMatch=112312, dots=50, q="dna", t="dna"). Be sure to only pass parameters accepted by either BLAT or gfClient. For example, if repMatch or stepSize parameters are specified when using gfClient, then the function will simply ignore them! The defaults are configured to align a 17bp sequence with 70% identity.
#'
...
blatSeqs <- function(query=NULL, subject=NULL, standaloneBlat=TRUE, port=NULL, host="localhost", parallel=TRUE, gzipResults=TRUE, blatParameters=c(minIdentity=70, minScore=5, stepSize=5, tileSize=8, repMatch=112312, dots=50, q="dna", t="dna")) {
